...
export default function Atis({data, i}: Props) {
    ...
    return (
        <Stack direction={'row'} padding={2} bgcolor={i % 2 == 0 ? 'palette.primary.light' : 'palette.primary.main'}>
            <Stack direction={'column'}>
                <Typography variant='body2' textAlign={'center'} width={'100%'}>{data.airport}</Typography>
                ...
                <div className={`${getFlightCategoryColor(atisParse.flightCategory)} relative w-18 h-18 flex justify-center items-center drop-shadow-md drop-shadow-black`}>
                    <Typography variant='h3' fontWeight={400}>{data.information}</Typography>
                </div>
                {atisParse.atisType == 'DEPARTURE' || atisParse.atisType == 'ARRIVAL' ?
                <Typography variant='caption' width={'100%'} textAlign={'center'}>{atisParse.atisType.toUpperCase()}</Typography>
                : ''}
            </Stack>
